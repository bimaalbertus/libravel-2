<div x-data="{
        ...
        <div class="group relative">
            @if ($user->avatar && $user->avatar->getFirstMediaUrl('avatars'))
                <img src="{{ $user->getAvatar() }}" alt="{{ $user->username }}'s avatar"
                    class="size-24 rounded-full object-cover cursor-pointer">
            @else
                {!! $user->getAvatar(96, 'circle') !!}
            @endif
    ...
    <x-modal open="openAvatar" :closeIcon="false" blur="none">
        <div x-cloak
            class="p-6 bg-light-bg-secondary dark:bg-dark-bg-secondary border border-black/30 dark:border-white/30 rounded-xl w-full">
            <h3 class="text-lg font-bold mb-4">Select Your Avatar</h3>
            <div class="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-4 gap-4 md:gap-8 mb-6">
                @foreach ($avatars as $avatar)
                    <label class="block cursor-pointer" wire:click="$set('selectedAvatarId', {{ $avatar->id }})">
                        ...
                        <img src="{{ $avatar->getFirstMediaUrl('avatars') }}" alt="Avatar"
                            class="size-24 md:size-32 rounded-md object-cover cursor-pointer transition transform hover:scale-105 {{ $selectedAvatarId === $avatar->id ? 'ring-4 ring-offset-1 ring-blue-500' : '' }}">
                    </label>
                @endforeach
            </div>
